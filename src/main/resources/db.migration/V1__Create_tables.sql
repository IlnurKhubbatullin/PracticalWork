...
create table schema_system.contractor_documents (contractors_id bigint not null, documents_id bigint not null);
...
create table schema_system.field (
    ...
    template_id bigint,
    primary key (id));
create table schema_system.file (
    id bigserial not null,
    created_at timestamp(6),
...
alter table if exists schema_system.document add constraint UK_raknd9chci5ty3rxg7sggi8qe unique (file_id);
alter table if exists schema_system.document add constraint UK_nltsk5ftwd1mmkc5s7i7t7ova unique (template_id);
